...
#include <rex/runtime/flags.h>
#include <rex/runtime/export_resolver.h>
#include <rex/runtime/function.h>
#include <rex/runtime/lzx.h>
...
int XexModule::ApplyPatch(XexModule* module) {
  ...
  if (new_image_size > original_image_size) {
    ...
    if (!alloc_result) {
      REXLOG_ERROR("Unable to allocate XEX memory at {:08X}-{:08X}.", addr_new_mem,
             size_delta);
      assert_always();
      return 6;
    }
  ...
  if (!result_code) {
    ...
    if (original_image_size > new_image_size) {
      ...
      if (!free_result) {
        REXLOG_ERROR("Unable to decommit XEX memory at {:08X}-{:08X}.", addr_free_mem,
               size_delta);
        assert_always();
      }
    }
    ...
    REXLOG_INFO(
        ...
        "version: {}.{}.{}.{}",
        (uint32_t)source_ver.major, (uint32_t)source_ver.minor,
        (uint32_t)source_ver.build, (uint32_t)source_ver.qfe,
        (uint32_t)target_ver.major, (uint32_t)target_ver.minor,
...
bool XexModule::Load(const std::string_view name, const std::string_view path,
  ...
  if (result_code) {
    REXLOG_WARN("XEX load failed with code {}, trying with devkit encryption key...",
           result_code);

    result_code = ReadImage(xex_addr, xex_length, true);
    if (result_code) {
      REXLOG_ERROR("XEX load failed with code {}, tried both encryption keys",
             result_code);
      return false;
    }
  }
...
bool XexModule::SetupLibraryImports(const std::string_view name,
  ...
  for (uint32_t i = 0; i < library->count; i++) {
    ...
    if (!record_addr) continue;

    auto record_slot =
        memory()->TranslateVirtual<rex::be<uint32_t>*>(record_addr);
    ...
    if (record_type == 0) {
      ...
      if (export_resolver) {
        ...
        if (kernel_export &&
            kernel_export->type == runtime::Export::Type::kVariable) {
          if (kernel_export->is_implemented() && kernel_export->variable_ptr) {
            ...
            *record_slot = kernel_export->variable_ptr;
            REXLOG_DEBUG("Patched variable import {}:{:#x} ({}) -> {:#x}",
                   base_name, ordinal, kernel_export->name,
                   kernel_export->variable_ptr);
          } else {
              // write garbage value if we don't have it implemented
              *record_slot = 0xD000BEEF | (kernel_export->ordinal & 0xFFF) << 16;
            REXLOG_WARN("Variable import {}:{:#x} ({}) not implemented",
                   base_name, ordinal, kernel_export->name);
          }
        }
      }
