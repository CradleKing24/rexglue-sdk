...
REXCVAR_DEFINE_BOOL(protect_zero, true,
    "Protect the zero page from reads and writes",
    "Memory")
    .lifecycle(rex::cvar::Lifecycle::kRequiresRestart);

REXCVAR_DEFINE_BOOL(protect_on_release, false,
    "Protect released memory to prevent accesses",
    "Memory");

REXCVAR_DEFINE_BOOL(scribble_heap, false,
...
Memory::Memory() {
  ...
  system_allocation_granularity_ =
      uint32_t(rex::memory::allocation_granularity());
...
bool Memory::Initialize() {
  ...
  if (mapping_ == rex::memory::kFileMappingHandleInvalid) {
    REXKRNL_ERROR("Unable to reserve the 4gb guest address space.");
    assert_always();
    return false;
  }
  ...
  if (!mapping_base_) {
    REXKRNL_ERROR("Unable to find a continuous block in the 64bit address space.");
    assert_always();
    return false;
  }
  ...
  heaps_.v00000000.Initialize(this, virtual_membase_, memory::HeapType::kGuestVirtual,
                              0x00000000, 0x40000000, 4096);
  heaps_.v40000000.Initialize(this, virtual_membase_, memory::HeapType::kGuestVirtual,
                              0x40000000, 0x40000000 - 0x01000000, 64 * 1024);
  heaps_.v80000000.Initialize(this, virtual_membase_, memory::HeapType::kGuestXex,
                              0x80000000, 0x10000000, 64 * 1024);
  heaps_.v90000000.Initialize(this, virtual_membase_, memory::HeapType::kGuestXex,
                              0x90000000, 0x10000000, 4096);

  // Prepare physical heaps.
  heaps_.physical.Initialize(this, physical_membase_, memory::HeapType::kGuestPhysical,
                             0x00000000, 0x20000000, 4096);
  heaps_.vA0000000.Initialize(this, virtual_membase_, memory::HeapType::kGuestPhysical,
                              0xA0000000, 0x20000000, 64 * 1024,
                              &heaps_.physical);
  heaps_.vC0000000.Initialize(this, virtual_membase_, memory::HeapType::kGuestPhysical,
                              0xC0000000, 0x20000000, 16 * 1024 * 1024,
                              &heaps_.physical);
  heaps_.vE0000000.Initialize(this, virtual_membase_, memory::HeapType::kGuestPhysical,
  ...
  heaps_.v00000000.AllocFixed(
      ...
      !REXCVAR_GET(protect_zero) ? memory::kMemoryProtectRead | memory::kMemoryProtectWrite
                           : memory::kMemoryProtectNoAccess);
  heaps_.physical.AllocFixed(0x1FFF0000, 0x10000, 0x10000,
                             memory::kMemoryAllocationReserve, memory::kMemoryProtectNoAccess);
  ...
  if (!mmio_handler_) {
    REXKRNL_ERROR("Unable to install MMIO handlers");
    assert_always();
    return false;
  }
  REXKRNL_DEBUG("Installed MMIO handler for physical address translation");
...
uint32_t Memory::HostToGuestVirtualThunk(const void* context,
  ...
  return reinterpret_cast<const memory::Memory*>(context)->HostToGuestVirtual(
      host_address);
}
...
bool Memory::AddVirtualMappedRange(uint32_t virtual_address, uint32_t mask,
    ...
    if (!rex::memory::AllocFixed(TranslateVirtual(virtual_address), size,
        ...
        REXKRNL_ERROR("Unable to map range; commit/protect failed");
        return false;
    }
...
void Memory::DumpMap() {
  REXKRNL_ERROR("==================================================================");
  REXKRNL_ERROR("Memory Dump");
  REXKRNL_ERROR("==================================================================");
  REXKRNL_ERROR("               System Page Size: {0} ({0:08X})", system_page_size_);
  REXKRNL_ERROR("  System Allocation Granularity: {0} ({0:08X})",
         system_allocation_granularity_);
  REXKRNL_ERROR("                Virtual Membase: {}", fmt::ptr(virtual_membase_));
  REXKRNL_ERROR("               Physical Membase: {}", fmt::ptr(physical_membase_));
  REXKRNL_ERROR("");
  REXKRNL_ERROR("------------------------------------------------------------------");
  REXKRNL_ERROR("Virtual Heaps");
  REXKRNL_ERROR("------------------------------------------------------------------");
  REXKRNL_ERROR("");
...
bool Memory::Save(stream::ByteStream* stream) {
  REXKRNL_DEBUG("Serializing memory...");
...
bool Memory::Restore(stream::ByteStream* stream) {
  REXKRNL_DEBUG("Restoring memory...");
...
bool Memory::InitializeFunctionTable(uint32_t code_base, uint32_t code_size,
  ...
  if (function_table_base_ != 0) {
    REXKRNL_ERROR("Function table already initialized");
    return false;
  }
  ...
  REXKRNL_DEBUG("Initializing function table at {:08X}, size {:08X} for code {:08X}-{:08X}",
...
void Memory::SetFunction(uint32_t guest_address, PPCFunc* host_function) {
  if (function_table_base_ == 0) {
    REXKRNL_ERROR("SetFunction called before InitializeFunctionTable");
    return;
  }
  ...
  if (guest_address < function_code_base_ ||
    ...
    REXKRNL_DEBUG("SetFunction: skipping {:08X} (outside code section [{:08X}, {:08X}))",
           guest_address, function_code_base_,
           function_code_base_ + function_code_size_);
    return;
  }
...
void BaseHeap::DumpMap() {
  ...
  REXKRNL_ERROR("------------------------------------------------------------------");
  REXKRNL_ERROR("Heap: {:08X}-{:08X}", heap_base_, heap_base_ + (heap_size_ - 1));
  REXKRNL_ERROR("------------------------------------------------------------------");
  REXKRNL_ERROR("            Heap Base: {:08X}", heap_base_);
  REXKRNL_ERROR("            Heap Size: {0} ({0:08X})", heap_size_);
  REXKRNL_ERROR("            Page Size: {0} ({0:08X})", page_size_);
  REXKRNL_ERROR("           Page Count: {}", page_table_.size());
  REXKRNL_ERROR("  Host Address Offset: {0} ({0:08X})", host_address_offset_);
  ...
  for (uint32_t i = 0; i < uint32_t(page_table_.size()); ++i) {
    ...
    if (is_empty_span) {
      REXKRNL_ERROR("  {:08X}-{:08X} {:6d}p {:10d}b unreserved",
    ...
    REXKRNL_ERROR("  {:08X}-{:08X} {:6d}p {:10d}b {} {}{}",
           heap_base_ + i * page_size_,
        heap_base_ + (i + region_pages) * page_size_,
        region_pages, region_pages * page_size_,
           state_name, access_r, access_w);
    i += region_pages - 1;
  }
  if (is_empty_span) {
    REXKRNL_ERROR("  {:08X}-{:08X} - {} unreserved pages)",
           heap_base_ + empty_span_start * page_size_,
...
bool BaseHeap::Save(stream::ByteStream* stream) {
  REXKRNL_DEBUG("Heap {:08X}-{:08X}", heap_base_, heap_base_ + (heap_size_ - 1));
...
bool BaseHeap::Restore(stream::ByteStream* stream) {
  ...
  for (size_t i = 0; i < page_table_.size(); i++) {
    ...
    if (page.state & memory::kMemoryAllocationCommit) {
      void* addr = TranslateRelative(i * page_size_);
      rex::memory::Protect(addr, page_size_, memory::PageAccess::kReadWrite,
                          nullptr);
...
bool BaseHeap::AllocFixed(uint32_t base_address, uint32_t size,
  ...
  if (start_page_number >= page_table_.size() ||
    ...
    REXKRNL_ERROR("BaseHeap::AllocFixed passed out of range address range");
    return false;
  }
  ...
  for (uint32_t page_number = start_page_number; page_number <= end_page_number;
    ...
    if ((allocation_type == memory::kMemoryAllocationCommit) &&
      ...
      // This may be OK.
      REXKRNL_WARN("BaseHeap::AllocFixed attempting commit on unreserved page");
...
bool BaseHeap::AllocRange(uint32_t low_address, uint32_t high_address,
  ...
  *out_address = 0;
  ...
  high_address = std::min(heap_base_ + (heap_size_ - 1),
                          rex::align(high_address, alignment));
  ...
  if (page_count > (high_page_number - low_page_number)) {
    REXKRNL_ERROR("BaseHeap::Alloc page count too big for requested range");
    return false;
  }
  ...
  if (top_down) {
    for (int64_t base_page_number =
             high_page_number - rex::round_up(page_count, page_scan_stride);
  ...
  if (start_page_number == UINT_MAX || end_page_number == UINT_MAX) {
    // Out of memory.
    REXKRNL_ERROR("BaseHeap::Alloc failed to find contiguous range");
  ...
  if (allocation_type == memory::kMemoryAllocationReserve) {
    ...
    void* result = rex::memory::AllocFixed(
        ...
        page_count * page_size_, alloc_type, ToPageAccess(protect));
    if (!result) {
      REXKRNL_ERROR("BaseHeap::Alloc failed to alloc range from host");
      return false;
    }
...
bool BaseHeap::Release(uint32_t base_address, uint32_t* out_region_size) {
  ...
  if (base_page_entry.base_address != base_page_number) {
    REXKRNL_ERROR("BaseHeap::Release failed because address is not a region start");
    return false;
  }

  if (heap_base_ == 0x00000000 && base_page_number == 0) {
    REXKRNL_ERROR("BaseHeap::Release: Attempt to free 0!");
    return false;
  }
  ...
  if (page_size_ == rex::memory::page_size() ||
    ...
    if (REXCVAR_GET(protect_on_release)) {
      if (!rex::memory::Protect(TranslateRelative(base_page_number * page_size_),
        ...
        REXKRNL_WARN("BaseHeap::Release failed due to host VirtualProtect failure");
      }
    }
  }
...
bool BaseHeap::Protect(uint32_t address, uint32_t size, uint32_t protect,
  ...
  if (!size) {
    REXKRNL_ERROR("BaseHeap::Protect failed due to zero size");
    return false;
  }
  ...
  uint32_t start_page_number = (address - heap_base_) / page_size_;
  if (start_page_number >= page_table_.size()) {
    REXKRNL_ERROR("BaseHeap::Protect failed due to out-of-bounds base address {:08X}",
           address);
    return false;
  }
  uint32_t end_page_number =
  ...
  for (uint32_t page_number = start_page_number; page_number <= end_page_number;
    ...
    if (first_base_address == UINT_MAX) {
      ...
      REXKRNL_ERROR("BaseHeap::Protect failed due to request spanning regions");
      return false;
    }
    if (!(page_entry.state & memory::kMemoryAllocationCommit)) {
      REXKRNL_ERROR("BaseHeap::Protect failed due to uncommitted page");
      return false;
    }
  }
  ...
  if (page_size_ == rex::memory::page_size() ||
    ...
    if (!rex::memory::Protect(TranslateRelative(start_page_number * page_size_),
      ...
      REXKRNL_ERROR("BaseHeap::Protect failed due to host VirtualProtect failure");
      return false;
    }
    ...
    REXKRNL_WARN("BaseHeap::Protect: ignoring request as not 4k page aligned");
    return false;
  }
...
bool BaseHeap::QueryRegionInfo(uint32_t base_address,
  ...
  uint32_t start_page_number = (base_address - heap_base_) / page_size_;
  if (start_page_number > page_table_.size()) {
    REXKRNL_ERROR("BaseHeap::QueryRegionInfo base page out of range");
    return false;
  }
...
bool BaseHeap::QuerySize(uint32_t address, uint32_t* out_size) {
  ...
  if (page_number > page_table_.size()) {
    REXKRNL_ERROR("BaseHeap::QuerySize base page out of range");
    *out_size = 0;
    return false;
  }
  auto global_lock = global_critical_region_.Acquire();
...
bool BaseHeap::QueryProtect(uint32_t address, uint32_t* out_protect) {
  uint32_t page_number = (address - heap_base_) / page_size_;
  if (page_number > page_table_.size()) {
    REXKRNL_ERROR("BaseHeap::QueryProtect base page out of range");
    *out_protect = 0;
    return false;
  }
  auto global_lock = global_critical_region_.Acquire();
...
rex::memory::PageAccess BaseHeap::QueryRangeAccess(uint32_t low_address,
...
void VirtualHeap::Initialize(memory::Memory* memory, uint8_t* membase,
...
void PhysicalHeap::Initialize(memory::Memory* memory, uint8_t* membase,
                              HeapType heap_type, uint32_t heap_base,
                              uint32_t heap_size, uint32_t page_size,
                              VirtualHeap* parent_heap) {
  uint32_t host_address_offset;
  if (heap_base >= 0xE0000000 &&
      rex::memory::allocation_granularity() > 0x1000) {
...
bool PhysicalHeap::Decommit(uint32_t address, uint32_t size) {
  ...
  if (!parent_heap_->Decommit(parent_address, size)) {
    REXKRNL_ERROR("PhysicalHeap::Decommit failed due to parent heap failure");
    return false;
  }
...
bool PhysicalHeap::Release(uint32_t base_address, uint32_t* out_region_size) {
  ...
  if (!parent_heap_->Release(parent_base_address, out_region_size)) {
    REXKRNL_ERROR("PhysicalHeap::Release failed due to parent heap failure");
    return false;
  }
...
bool PhysicalHeap::Protect(uint32_t address, uint32_t size, uint32_t protect,
  ...
  if (!parent_heap_->Protect(GetPhysicalAddress(address), size, protect,
                             old_protect)) {
    REXKRNL_ERROR("PhysicalHeap::Protect failed due to parent heap failure");
    return false;
  }
...
void PhysicalHeap::EnableAccessCallbacks(uint32_t physical_address,
  ...
  rex::memory::PageAccess protect_access =
      enable_data_providers ? rex::memory::PageAccess::kNoAccess
...
bool PhysicalHeap::TriggerCallbacks(
  ...
  uint32_t physical_address_start =
      rex::sat_sub(system_page_first * system_page_size_,
                  host_address_offset()) +
  ...
  if (unprotect) {
    ...
    for (uint32_t i = system_page_first; i <= system_page_last; ++i) {
      ...
      if (unprotect_page) {
        uint32_t guest_page_number =
            rex::sat_sub(i * system_page_size_, host_address_offset()) /
            page_size_;
