...
#include <rex/runtime/elf_module.h>
#include <rex/runtime.h>
#include <rex/kernel/kernel_state.h>
#include <rex/kernel/xfile.h>
#include <rex/kernel/xthread.h>

REXCVAR_DEFINE_BOOL(xex_apply_patches, true,
    "Apply XEX patches",
    "Kernel");
...
X_STATUS UserModule::LoadFromFile(const std::string_view path) {
  ...
  if (!fs_entry) {
    REXKRNL_ERROR("File not found: {}", path);
  ...
  if (REXCVAR_GET(xex_apply_patches)) {
    ...
    if (patch_entry) {
      ...
      REXKRNL_DEBUG("Loading XEX patch from {}", patch_path);
      ...
      if (!result) {
        ...
        if (result) {
          REXKRNL_ERROR("Failed to apply XEX patch, code: {}", result);
        }
      } else {
        REXKRNL_ERROR("Failed to load XEX patch, code: {}", result);
      }
...
X_STATUS UserModule::LoadFromMemory(const void* addr, const size_t length) {
  ...
  if (magic == runtime::kXEX2Signature || magic == runtime::kXEX1Signature) {
    ...
    REXKRNL_ERROR("Unknown module magic: {:08X}", uint32_t(magic));
...
X_STATUS UserModule::GetSection(const std::string_view name,
  ...
  if (!runtime::XexModule::GetOptHeader(xex_header(), XEX_HEADER_RESOURCE_INFO,
                                    &resource_header)) {
    // No resources.
...
X_STATUS UserModule::GetOptHeader(const memory::Memory* memory,
...
object_ref<UserModule> UserModule::Restore(KernelState* kernel_state,
  ...
  if (XFAILED(result)) {
    REXKRNL_DEBUG("UserModule::Restore LoadFromFile({}) FAILED - code {:08X}", path,
           result);
    return nullptr;
  }
