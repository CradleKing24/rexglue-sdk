...
REXCVAR_DEFINE_BOOL(headless, false,
...
class XamDialog : public rex::ui::ImGuiDialog {
 ...
 protected:
  XamDialog(rex::ui::ImGuiDrawer* imgui_drawer)
...
X_RESULT xeXamDispatchDialog(T* dialog,
  ...
  auto run = [dialog, close_callback]() -> X_RESULT {
    ...
    rex::ui::WindowedAppContext* app_context =
        kernel_state()->emulator()->app_context();
    if (app_context && app_context->CallInUIThreadSynchronous(
...
class MessageBoxDialog : public XamDialog {
 public:
  MessageBoxDialog(rex::ui::ImGuiDrawer* imgui_drawer, std::string title,
...
dword_result_t XamShowMessageBoxUI_entry(
    ...
    dword_t flags, lpdword_t result_ptr, lpvoid_t overlapped) {
  REXKRNL_DEBUG("XamShowMessageBoxUI({:08X}, {:08X}, {:08X}, {:08X}, {:08X}, {:08X}, {:08X}, {:08X}, {:08X})",
      uint32_t(user_index), title_ptr.guest_address(), text_ptr.guest_address(),
      uint32_t(button_count), button_ptrs.guest_address(), uint32_t(active_button),
      uint32_t(flags), result_ptr.guest_address(), overlapped.guest_address());
  std::string title;
  if (title_ptr) {
  ...
  if (REXCVAR_GET(headless)) {
    ...
    if (imgui_drawer) {
      result = xeXamDispatchDialog<MessageBoxDialog>(
          new MessageBoxDialog(imgui_drawer, title, rex::string::to_utf8(text_ptr.value()),
...
class KeyboardInputDialog : public XamDialog {
 public:
  KeyboardInputDialog(rex::ui::ImGuiDrawer* imgui_drawer, std::string title,
    ...
    rex::string::util_copy_truncating(text_buffer_.data(), default_text_,
                                      text_buffer_.size());
  }
...
dword_result_t XamShowKeyboardUI_entry(
    ...
    dword_t buffer_length, lpvoid_t overlapped) {
  ...
  if (REXCVAR_GET(headless)) {
    auto run = [default_text, buffer, buffer_length,
      ...
      if (!default_text) {
        ...
        rex::string::util_copy_and_swap_truncating(buffer, default_text.value(),
                                              buffer_length);
      }
      return X_ERROR_SUCCESS;
    };
    ...
    if (imgui_drawer) {
      result = xeXamDispatchDialogEx<KeyboardInputDialog>(
          new KeyboardInputDialog(
              imgui_drawer, title ? rex::string::to_utf8(title.value()) : "",
...
GUEST_FUNCTION_HOOK(__imp__XamIsUIActive, rex::kernel::xam::XamIsUIActive_entry)
GUEST_FUNCTION_HOOK(__imp__XamShowMessageBoxUI, rex::kernel::xam::XamShowMessageBoxUI_entry)
GUEST_FUNCTION_HOOK(__imp__XamShowKeyboardUI, rex::kernel::xam::XamShowKeyboardUI_entry)
GUEST_FUNCTION_HOOK(__imp__XamShowDeviceSelectorUI, rex::kernel::xam::XamShowDeviceSelectorUI_entry)
GUEST_FUNCTION_HOOK(__imp__XamShowDirtyDiscErrorUI, rex::kernel::xam::XamShowDirtyDiscErrorUI_entry)
