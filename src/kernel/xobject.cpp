...
#include <rex/kernel/util/string_utils.h>  // For TranslateAnsiStringAddress
...
rex::Runtime* XObject::emulator() const { return kernel_state_->emulator_; }
KernelState* XObject::kernel_state() const { return kernel_state_; }
rex::memory::Memory* XObject::memory() const { return kernel_state_->memory(); }
...
void XObject::SetAttributes(uint32_t obj_attributes_ptr) {
  ...
  auto name = util::TranslateAnsiStringAddress(
      memory(), memory::load_and_swap<uint32_t>(
...
X_STATUS XObject::Wait(uint32_t wait_reason, uint32_t processor_mode,
  ...
  auto timeout_ms =
      opt_timeout ? std::chrono::milliseconds(chrono::Clock::ScaleGuestDurationMillis(
                        TimeoutTicksToMs(*opt_timeout)))
                  : std::chrono::milliseconds::max();

  auto result =
      rex::thread::Wait(wait_handle, alertable ? true : false, timeout_ms);
  switch (result) {
...
X_STATUS XObject::SignalAndWait(XObject* signal_object, XObject* wait_object,
  ...
  auto result = rex::thread::SignalAndWait(
...
X_STATUS XObject::WaitMultiple(uint32_t count, XObject** objects,
  ...
  if (wait_type) {
    auto result = rex::thread::WaitAny(std::move(wait_handles),
                                         alertable ? true : false, timeout_ms);
    switch (result.first) {
      case rex::thread::WaitResult::kSuccess:
    ...
    auto result = rex::thread::WaitAll(std::move(wait_handles),
                                         alertable ? true : false, timeout_ms);
    switch (result) {
      case rex::thread::WaitResult::kSuccess:
