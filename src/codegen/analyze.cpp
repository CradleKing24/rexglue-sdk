...
std::optional<ParsedExceptionInfo> parseSehScopeTable(
    uint32_t handlerThunk,
    uint32_t scopeTableAddr,
    uint32_t count,
    const uint8_t* rdataBase,
    uint32_t rdataStart,
    uint32_t rdataSize,
    ...
    std::vector<uint32_t>& discoveredFuncs)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (scope.tryStart != 0 && scope.tryStart > maxAddr) maxAddr = scope.tryStart;
        if (scope.tryEnd != 0 && scope.tryEnd > maxAddr) maxAddr = scope.tryEnd;
        if (scope.filter != 0 && scope.filter > maxAddr) maxAddr = scope.filter;
        if (scope.handler != 0 && scope.handler > maxAddr) maxAddr = scope.handler;
...
std::optional<ParsedExceptionInfo> parseCxxFuncInfo(
    uint32_t handlerThunk,
    ...
    std::vector<uint32_t>& discoveredFuncs)
{
    ...
    if (pIPtoStateMap != 0 && nIPMapEntries > 0) {
        if (pIPtoStateMap >= rdataStart && pIPtoStateMap + nIPMapEntries * 8 <= rdataStart + rdataSize) {
...
std::optional<ParsedExceptionInfo> parseExceptionInfo(
...
VoidResult registerEntryPoints(CodegenContext& ctx, std::vector<uint32_t>& ehDiscoveredFuncs) {
    ...
    // Register imports
    {
        ...
        for (const auto& sym : binary.importSymbols()) {
            ...
            if (resolver) {
                ...
                if (!exp) exp = resolver->GetExportByOrdinal(lib_name, ordinal);
    ...
    auto registerHelpers = [&](uint32_t base14, const char* prefix, size_t stride,
                               size_t endReg, size_t extraSize) {
        if (base14 == 0) return;
            ...
            if (exInfo) {
                ...
                if (exInfo->info.isSeh()) {
                    ...
                    if (!prologInfo.valid && !sehInfo.scopes.empty()) {
                        REXCODEGEN_WARN("SEH function 0x{:08X}: could not determine frame size from prolog", beginAddr);
                    }
                    ...
                    if (prologInfo.saveHelper != 0 &&
                        state.restGpr14Address != 0 &&
                        state.saveGpr14Address != 0) {
                        sehInfo.restoreHelper = prologInfo.saveHelper +
                            (state.restGpr14Address - state.saveGpr14Address);
                    }
                }
            }
        }
        ...
        if (exInfo && exInfo->info.hasInfo()) {
            if (auto* seh = exInfo->info.asSeh()) {
                for (const auto& scope : seh->scopes) {
                    if (scope.tryStart != 0) graph.addLabelToFunction(beginAddr, scope.tryStart);
                    if (scope.tryEnd != 0) graph.addLabelToFunction(beginAddr, scope.tryEnd);
                ...
                for (const auto& entry : cxx->ipToStateMap) {
                    if (entry.ip != 0) graph.addLabelToFunction(beginAddr, entry.ip);
                }
            }
    ...
    for (uint32_t addr : ehDiscoveredFuncs) {
        if (graph.getFunction(addr) != nullptr) continue;
        if (graph.isImport(addr)) continue;
...
std::vector<CodeRegion> segmentSection(
    ...
    if (inCode) {
        uint32_t endAddr = section.baseAddress +
                           static_cast<uint32_t>(dataEnd - section.data);
...
void scanBinary(CodegenContext& ctx) {
    ...
    for (const auto& section : binary.sections()) {
        ...
        scan.codeRegions.insert(scan.codeRegions.end(),
    ...
    for (const auto& section : binary.sections()) {
        ...
        while (data < dataEnd) {
            ...
            if (isInvalid) {
                if (consecutiveInvalid == 0) dataRegionStart = addr;
                consecutiveInvalid++;
            } else {
    ...
    REXCODEGEN_INFO("Analyze: {} code regions, {} data regions",
                   scan.codeRegions.size(), scan.dataRegions.size());
}
    ...
    const std::unordered_set<uint32_t>& knownFunctions)
{
    ...
    node->discover(std::move(result.blocks),
    ...
    for (const auto& branch : result.unresolvedBranches) {
        graph.addUnresolvedJumpToFunction(funcAddr, branch.site, branch.target,
                                           branch.isCall, branch.isConditional);
    }
    ...
    if (pdataSize > 0) {
        ...
        if (funcData) {
            for (uint32_t offset = 0; offset < pdataSize; offset += 4) {
                ...
                if (discoveredAddrs.count(site)) continue;
                ...
                if (opcode != PPC_OP_B && opcode != PPC_OP_BC) continue;
...
void discoverAllFunctions(CodegenContext& ctx) {
    ...
    while (iteration < MAX_ITERATIONS) {
        ...
        if (currentFunctionCount == lastFunctionCount && iteration > 1) {
            REXCODEGEN_DEBUG("Analyze: fixed point at iteration {} ({} functions)",
                            iteration, currentFunctionCount);
            break;
        }
    ...
    // VTable scanning
    {
        ...
        for (const auto& vt : vtables) {
            for (size_t i = 0; i < vt.slots.size(); i++) {
                ...
                if (graph.isEntryPoint(funcAddr)) continue;
                if (binary.isInImportExportRange(funcAddr)) continue;
        ...
        REXCODEGEN_INFO("Analyze: VTable scan found {} vtables, {} new functions",
                       vtables.size(), newFunctions);
    ...
    for (const auto& region : codeRegions) {
        ...
        for (uint32_t addr = region.start; addr < region.end; addr += 4) {
            ...
            if (insn->opcode == rex::codegen::ppc::Opcode::addi) {
                ...
                if (ra == 0) continue;  // li pseudo-op, not addi

                auto it = lisValues.find(ra);
                if (it == lisValues.end()) continue;
                ...
                if (existingFunctions.contains(fullAddr)) continue;
                ...
                REXCODEGEN_TRACE("functionPointerScan: found 0x{:08X} via lis/addi at 0x{:08X}",
                                fullAddr, addr);
            }
            ...
            if (insn->opcode == rex::codegen::ppc::Opcode::ori) {
                ...
                int32_t distance = static_cast<int32_t>(fullAddr) - static_cast<int32_t>(addr);
                if (distance > -0x1000 && distance < 0x1000) continue;

                graph.addFunction(fullAddr, 4, FunctionAuthority::DISCOVERED, true);
                existingFunctions.insert(fullAddr);
                foundCount++;

                REXCODEGEN_TRACE("functionPointerScan: found 0x{:08X} via lis/ori at 0x{:08X}",
                                fullAddr, addr);
            }
    ...
    for (uint32_t addr = region.start; addr < region.end; addr += 4) {
        ...
        if (shouldSplit) {
            ...
            if (segmentEnd > segmentStart) {
                ...
                REXCODEGEN_TRACE("GapFill: split segment 0x{:08X}-0x{:08X} ({} at 0x{:08X})",
                                segmentStart, segmentEnd, reason, addr);
            }
...
bool looksLikeExceptionData(const BinaryView& binary, const FunctionGraph& graph, uint32_t addr) {
    ...
    auto* rdataSection = binary.findSectionByName(".rdata");
    if (!rdataSection) return false;

    uint32_t rdataStart = rdataSection->baseAddress;
    uint32_t rdataEnd = rdataStart + rdataSection->size;

    if (secondDword >= rdataStart && secondDword < rdataEnd) {
        REXCODEGEN_TRACE("GapFill: 0x{:08X} looks like exception data (handler=0x{:08X}, scope=0x{:08X}), skipping",
...
void gapFillCodeRegions(CodegenContext& ctx) {
    ...
    for (const auto& region : scan.codeRegions) {
        ...
        for (const auto& segment : segments) {
            ...
            if (graph.isEntryPoint(segment.start)) continue;
            ...
            if (looksLikeExceptionData(binary, graph, segment.start)) continue;
    ...
    if (segmentsCreated > 0) {
        REXCODEGEN_INFO("Analyze: registered {} gap functions from {} regions", segmentsCreated, gapsFound);
    } else {
...
void mergeAndSeal(CodegenContext& ctx) {
    ...
    while (iteration < MAX_ITERATIONS) {
        ...
        if (changesThisIteration == 0) break;
    }

    size_t totalSealed = graph.sealAllReady();
    size_t stillPending = graph.pendingCount();

    REXCODEGEN_INFO("Analyze: {} iterations, resolved={}, sealed={}/{}",
                   iteration, totalResolved, totalSealed, graph.functionCount());
...
VoidResult validateGraph(CodegenContext& ctx) {
            ...
            for (size_t offset = 0; offset < block.size; offset += 4) {
                ...
                if (PPC_OP(insn) == PPC_OP_B && !PPC_BA(insn)) {
                    ...
                    if (!targetExists) {
                        ...
                        errors.Add(AnalysisErrors::Category::UnresolvedCall,
                                  target, site,
    ...
    REXCODEGEN_INFO("Analyze: checked {} branches in {} functions, verified {} edges",
                   callsChecked, functionsChecked, edgesVerified);
...
Result<void> Analyze(CodegenContext& ctx) {
    ...
    // (They need their blocks discovered too... don't be selfish)
    {
        ...
        REXCODEGEN_INFO("Analyze: discovered blocks for {} gap-filled functions", needsDiscovery.size());
    }
...
const char* AnalysisErrors::CategoryName(Category cat) {
    switch (cat) {
        case Category::UnresolvedCall:        return "UnresolvedCall";
        case Category::MissingJumpTable:      return "MissingJumpTable";
        case Category::JumpTargetOutOfBounds: return "JumpTargetOutOfBounds";
        case Category::DiscontinuousFunction: return "DiscontinuousFunction";
        case Category::UnimplementedInsn:     return "UnimplementedInsn";
...
void AnalysisErrors::Add(Category cat, uint32_t addr, uint32_t secondary,
    ...
    for (const auto& [cat, entries] : byCategory) {
        ...
        for (const auto* entry : entries) {
            if (entry->secondaryAddress != 0) {
                REXCODEGEN_ERROR("  0x{:08X} from 0x{:08X}: {}",
                                entry->address, entry->secondaryAddress,
                                entry->message);
            } else {
