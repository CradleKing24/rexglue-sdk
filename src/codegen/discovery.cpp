...
bool isBlockTerminator(const DecodedInsn& insn, uint32_t addr,
    ...
    if (isBranch(insn) && !isConditional(insn) && !isCall(insn)) {
        ...
        if (target) {
            ...
            if (!region.contains(*target)) return true;
            // Branch to known function is tail call (terminator)
            if (knownFunctions.contains(*target)) return true;
        }
...
BoundsInfo scanForBounds(DecodedBinary& decoded, uint32_t bctrAddr,
                         const CodeRegion& region, uint8_t expectedReg) {
    BoundsInfo result;
    ...
    for (int i = 0; i < kMaxBackwardScan && scanAddr >= region.start + 4; i++) {
        ...
        if (insn->opcode == Opcode::cmpli) {
            ...
            REXCODEGEN_TRACE("scanForBounds: found cmpli at 0x{:08X} RA=r{} UIMM={} (expecting r{})",
                            scanAddr, static_cast<unsigned>(insn->D.RA), static_cast<int>(insn->D.d), expectedReg);
        ...
        if (insn->opcode == Opcode::rlwinm) {
            if (insn->M.RA == expectedReg && insn->M.SH == 0 && insn->M.ME == 31 && insn->M.MB > 0) {
                ...
                result.indexReg = expectedReg;
                result.found = true;
                REXCODEGEN_TRACE("scanForBounds: found clrlwi at 0x{:08X} MB={} maxEntries={}",
                                scanAddr, static_cast<unsigned>(insn->M.MB), result.maxEntries);
                return result;
            }
        }
    }
...
std::optional<JumpTable> detectJumpTable(
    ...
    for (int i = 0; i < kMaxBackwardScan && scanAddr >= containingRegion.start + 4; i++) {
        ...
        // Stop at unconditional terminators (but NOT conditional branches - they're often bounds checks)
        if (isTerminator(*insn) && !isConditional(*insn)) break;
        ...
        if (foundMtctr && !foundLoad) {
            ...
            if (insn->opcode == Opcode::lwzx && insn->X.RT == ctrSourceReg) {
                ...
                foundLoad = true;
                REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} found lwzx at 0x{:08X}", bctrAddr, scanAddr);
                continue;
            }
            ...
            if (insn->opcode == Opcode::lbzx && insn->X.RT == ctrSourceReg) {
                ...
                finalIndexReg = indexReg;
                foundLoad = true;
                REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} found lbzx at 0x{:08X}", bctrAddr, scanAddr);
                continue;
            }
            ...
            if (insn->opcode == Opcode::lhzx && insn->X.RT == ctrSourceReg) {
                ...
                finalIndexReg = indexReg;
                foundLoad = true;
                REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} found lhzx at 0x{:08X}", bctrAddr, scanAddr);
                continue;
            }
            ...
            if (insn->opcode == Opcode::rlwinm && insn->M.RA == ctrSourceReg) {
                ...
                REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} found rlwinm at 0x{:08X}", bctrAddr, scanAddr);
                continue;
            }

            // Log unhandled instructions in the chain (potential issue)
            REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} unhandled insn at 0x{:08X} opcode={} while looking for load into r{}",
        ...
        if (foundLoad && indexReg != 0xFF) {
            ...
            if (insn->opcode == Opcode::rlwinm) {
                ...
                destReg = insn->D.RT;
                       ...
                       insn->opcode == Opcode::or_ || insn->opcode == Opcode::and_ ||
                       insn->opcode == Opcode::xor_) {
            ...
            if (destReg == indexReg) {
                ...
                if (insn->opcode == Opcode::rlwinm) {
                    ...
                    if (sh > 0 && mb == 0 && me == (31 - sh)) {
                        indexReg = insn->M.RS;
                        finalIndexReg = indexReg;
                        REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} found slwi at 0x{:08X} indexReg now r{}",
                                        bctrAddr, scanAddr, indexReg);
                    } else {
                        // Non-slwi rlwinm writes to indexReg, stop tracing
                        REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} indexReg r{} overwritten by non-slwi rlwinm at 0x{:08X}, stop tracing",
                    ...
                    REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} indexReg r{} overwritten at 0x{:08X}, stop tracing",
                                    bctrAddr, indexReg, scanAddr);
                    indexReg = 0xFF; // Mark as invalid to stop further tracing
                }
            }
        ...
        if (foundMtctr) {
            ...
            if (insn->opcode == Opcode::lis) {
                ...
                REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} found lis at 0x{:08X} hi=0x{:08X} foundLoad={}",
                                bctrAddr, scanAddr, hi, foundLoad);
                if (foundLoad) {
                    // After load: this is tableAddr (only capture first complete address)
                    if (tableAddr == 0) {
                        tableAddr = hasPendingTableLo ? combineHiLo(hi, pendingTableLo, pendingTableLoIsAddi) : hi;
                        hasPendingTableLo = false;
                        REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} set tableAddr=0x{:08X}", bctrAddr, tableAddr);
                    }
                    ...
                    if (baseAddr == 0) {
                        ...
                        REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} set baseAddr=0x{:08X}", bctrAddr, baseAddr);
                    }
            ...
            if (insn->opcode == Opcode::addi || insn->opcode == Opcode::ori) {
                ...
                REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} found {} at 0x{:08X} lo=0x{:04X} foundLoad={}",
                ...
                if (foundLoad) {
                    ...
                    if (tableAddr == 0) {
                        if (hasPendingTableHi) {
                            ...
                            REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} set tableAddr=0x{:08X} from pending", bctrAddr, tableAddr);
                        } else if (!hasPendingTableLo) {
                            pendingTableLo = lo;
                            pendingTableLoIsAddi = isAddi;
                            hasPendingTableLo = true;
                            REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} pending tableLo=0x{:04X}", bctrAddr, lo);
                        }
                    } else if ((tableAddr & 0xFFFF) == 0) {
                        // tableAddr has only high bits, add low bits
                        tableAddr = combineHiLo(tableAddr, lo, isAddi);
                        REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} combined tableAddr=0x{:08X}", bctrAddr, tableAddr);
                    }
                    ...
                    if (baseAddr == 0) {
                        if (hasPendingBaseHi) {
                            ...
                            REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} set baseAddr=0x{:08X} from pending", bctrAddr, baseAddr);
                        } else if (!hasPendingBaseLo) {
                            pendingBaseLo = lo;
                            pendingBaseLoIsAddi = isAddi;
                            hasPendingBaseLo = true;
                            REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} pending baseLo=0x{:04X}", bctrAddr, lo);
                        }
                    } else if ((baseAddr & 0xFFFF) == 0) {
                        baseAddr = combineHiLo(baseAddr, lo, isAddi);
                        REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} combined baseAddr=0x{:08X}", bctrAddr, baseAddr);
                    }
    ...
    REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} scan complete: foundMtctr={} foundLoad={} tableAddr=0x{:08X} baseAddr=0x{:08X}",
                    bctrAddr, foundMtctr, foundLoad, tableAddr, baseAddr);

    if (!foundMtctr || !foundLoad || tableAddr == 0) {
        REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} FAILED foundMtctr={} foundLoad={} tableAddr=0x{:08X}",
    ...
    REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} reading {} entries from table=0x{:08X} base=0x{:08X} type={}",
    ...
    for (uint32_t i = 0; i < entryCount; i++) {
        ...
        if (target == 0 || !containingRegion.contains(target)) {
            ...
            REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} entry[{}] target=0x{:08X} invalid (region 0x{:08X}-0x{:08X})",
        ...
        if (target < funcStart) {
            REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} entry[{}] target=0x{:08X} < funcStart=0x{:08X}",
                            bctrAddr, i, target, funcStart);
            if (jt.targets.empty()) return std::nullopt;
            break;
        }
    ...
    if (jt.targets.empty()) {
        REXCODEGEN_TRACE("detectJumpTable: bctr=0x{:08X} table=0x{:08X} NO VALID TARGETS (funcStart=0x{:08X} funcEnd=0x{:08X})",
...
BlockDiscoveryResult discoverBlocks(
    ...
    REXCODEGEN_TRACE("discoverBlocks: entry=0x{:08X} pdataSize={} funcEnd=0x{:08X} region=[0x{:08X}-0x{:08X}]",
    ...
    while (!worklist.empty()) {
        ...
        if (visited.contains(blockStart)) continue;
        if (!isWithinFunction(blockStart)) continue;
    ...
    result.instructions.erase(
        std::unique(result.instructions.begin(), result.instructions.end()),
        result.instructions.end());

    REXCODEGEN_TRACE("discoverBlocks: entry=0x{:08X} blocks={} instructions={} labels={}",
                    entryPoint, result.blocks.size(), result.instructions.size(),
                    result.internalLabels.size());
